...
async function getLogGroup ({ FunctionName }) {
    const logGroupName = `/aws/lambda/${FunctionName}`
    const params = {
      logGroupNamePrefix: logGroupName
    }
    const groups = await cwLogs.describeLogGroups(params).promise()
    const logGroups = groups.logGroups;
    const matchedGroups = logGroups.filter((logGroup) => logGroup.logGroupName === logGroupName)
...
async function report(searchFunction, stateMachineName, monitorUniqueName) {
    ...
    for(let event of historyResponse.events) {
    ...
    do {
        ...
        for(let logStream of streamsResponse.logStreams) {
...
async function main() {
    ...
    let dispatchFunction = identifier+"-searchDispatch";
    let searchFunction = identifier+"-search";
    let stateMachineName = "searchMonitorStateMachine-"+identifier;

    // const readFile = util.promisify(fs.readFile);
    // let searchParamsJson = await readFile(infile, 'utf8');
    // let searchParams = JSON.parse(searchParamsJson);
    ...
    // let buff = Buffer.from(cdsInvocationResult.LogResult, 'base64');
    // let logLines = buff.toString('ascii');
    ...
    // report(response.monitorUniqueName);

    // const logStream = logStreams[0];
    // const logStreamNames = [logStream.logStreamName];
    // const eventsParams = {
    //     logGroupName,
    //     logStreamNames,
    //     startTime: stateMachineStarted.getTime()-86400000,
    //     interleaved: true
    // };
    //console.log(eventsParams);
    // const eventsResponse = await cwLogs.filterLogEvents(eventsParams).promise();
    // const logEvents = eventsResponse.events;
