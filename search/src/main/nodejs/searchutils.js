...
export function getSearchKey(searchInputName, ext) {
    if (!ext) {
...
export function getSearchMaskId(searchInputName, extParam) {
    const searchInputKey = getKeyWithNoExt(searchInputName);
...
export function getSearchMetadataKey(searchInputName) {
    const searchInputKey = getKeyWithNoExt(searchInputName);
    return `${searchInputKey}.metadata`;
}

// Replaces searchInputName's extension (if any) with ".result"
export function getSearchResultsKey(searchInputName) {
    const searchInputKey = getKeyWithNoExt(searchInputName);
...
export function getSearchSubFolder(searchInputName, folderName) {
    const searchInputKey = getKeyWithNoExt(searchInputName);
    const searchInputPathComps = searchInputKey.split('/');
...
export const getIntermediateSearchResultsPrefix = (searchInputName) => getSearchSubFolder(searchInputName, 'results');

// Get the key to results/batch_{batchNumber}.json relative to search input
export function getIntermediateSearchResultsKey(searchInputName, batchNumber) {
